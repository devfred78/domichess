...
def make_frame():
    ...
    my_combo = sg.Combo(
        my_list, default_value=my_list[0], enable_events=True, key="-MY_COMBO-"
    )
    my_text = sg.Text("This is my choice:")
    my_input = sg.Input("My choice here", key="-MY_CHOICE-")
    my_slider = sg.Slider(
        range=(0, 4), default_value=2, orientation="horizontal", key="-MY_SLIDER-"
    )
    my_frame = sg.Frame("My frame", [[my_combo, my_text, my_input], [my_slider]])

    window = sg.Window(
        "My window", [[my_frame]], finalize=True, enable_close_attempted_event=True
    )
...
def make_frame_with_tab():
    ...
    my_tab2 = sg.Tab(
        "Tab2",
        [[my_combo1, my_text1, my_input1], [my_slider1, sg.Push()], [my_button1]],
        key="-TAB2-",
    )
    my_tab3 = sg.Tab("Tab3", [[sg.Text("This tab is empty")]], key="-TAB3-")
    my_tab4 = sg.Tab(
        "Tab4",
        [[my_combo2, my_text2, my_input2], [my_slider2, sg.Push()], [my_button2]],
        key="-TAB4-",
    )
    my_tabgroup1 = sg.TabGroup(
        [[my_tab1, my_tab2]], key="-MY_TABGROUP1-", enable_events=True
    )
    my_tabgroup2 = sg.TabGroup(
        [[my_tab3, my_tab4]], key="-MY_TABGROUP2-", enable_events=True
    )
...
def main():
    ...
    while True:
        ...
        if event == "-USE CUSTOM TITLEBAR-":
            ...
            sg.popup_scrolled(
                sg.get_versions(), __file__, keep_on_top=True, non_blocking=True
            )
    window.close()
