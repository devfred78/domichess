...
from domichess import DEFAULT_WHITE_LOGO_PIECE, DEFAULT_BLACK_LOGO_PIECE, DEFAULT_ICON_PIECE, DEFAULT_ICON_SIZE, DEFAULT_LOGO_SIZE, DEFAULT_BOARD_SIZE, CHESS_DEFAULT_MARGIN_SIZE, CHESS_DEFAULT_SQUARE_SIZE, CHESS_DEFAULT_BOARD_SIZE, ENGINE_PATH, HELP_DURATION, DEFAULT_SEARCH_TIME, RESOURCE_PATH, DEFAULT_CURRENT_PLAYER_FILE, DEFAULT_CURRENT_PLAYER_SIZE
from domichess import Type, Key, KeyPopup, Draw, EngineProtocol, UCIOptionType, EngineFamily, DefaultText, DefaultBoardColor
...
class Display():
	...
	def __new__(cls, version = None, engine_path = pathlib.Path(ENGINE_PATH).resolve(), logger = None):
		"""
	...
	def __init__(self, version = None, engine_path = pathlib.Path(ENGINE_PATH).resolve(), logger = None):
		"""
		...
		with open(pathlib.Path(__file__).parent / pathlib.Path(RESOURCE_PATH) / pathlib.Path(DEFAULT_CURRENT_PLAYER_FILE), mode = 'rt') as current_neutral_svg_file:
			self._current_neutral_png_bytes = self.svg_to_png_bytes(current_neutral_svg_file.read())
		self._current_white_png_bytes = self.svg_to_png_bytes(chess.svg.piece(chess.Piece.from_symbol(self._white_logo_piece), size = DEFAULT_CURRENT_PLAYER_SIZE))
		self._current_black_png_bytes = self.svg_to_png_bytes(chess.svg.piece(chess.Piece.from_symbol(self._black_logo_piece), size = DEFAULT_CURRENT_PLAYER_SIZE))
		# Engines
		...
		if len(self.engine_list) != 0:
			memo = dict()
			if len(self.engine_list) == 1: # Only one engine: it will be also the helper engine
				self._helper_engine = deepcopy(self.engine_list[0], memo)
			else:
				self._helper_engine = deepcopy(random.choice(self.engine_list), memo) # The helper engine is chosen randomly
			...
			self.log.warning("No chess engine found. Helper and CPU opponents are unavailable.")
	...
	def _init_display(self):
		...
		player_image = sg.Image(self._current_neutral_png_bytes, key = Key.CURRENT_PLAYER_IMAGE)
		
		# logos images definitions
		white_logo_svg_str = chess.svg.piece(chess.Piece.from_symbol(self._white_logo_piece), size = self._logo_size)
		white_logo_image = self.svg_to_img(white_logo_svg_str, key = Key.WHITE_LOGO)
		black_logo_svg_str = chess.svg.piece(chess.Piece.from_symbol(self._black_logo_piece), size = self._logo_size)
		black_logo_image = self.svg_to_img(black_logo_svg_str, key = Key.BLACK_LOGO)
		
		# Engine list caption definition
		engine_caption_list = [engine.name for engine in self.engine_list] if len(self.engine_list) != 0 else None
		
		# White player tab definition
		# ---------------------------
		# Human tab definition
		white_human_tab = sg.Tab(Text.HUMAN, [[sg.Text(Text.WHITE_PLAYER_NAME)],
		...
		if engine_caption_list:
			...
			for engine in self.engine_list:
				...
				self.engine_options[chess.WHITE][engine.name] = engine.option.copy_without_engine(memo)
				self.log.debug(f"{engine.name} copied.")
			default_engine = self.engines[chess.WHITE][engine_caption_list[0]]
			white_cpu_combo = sg.Combo(engine_caption_list, default_value = default_engine.name, enable_events = True, key = Key.WHITE_ENGINE_COMBO, readonly = True)
			white_cpu_duration = sg.Input(default_text = DEFAULT_SEARCH_TIME, size = 5, key = Key.WHITE_CPU_DURATION)
			...
			if 'UCI_Elo' in default_engine.engine_options:
				if 'UCI_LimitStrength' in default_engine.engine_options:
					if not self.engine_options[chess.WHITE][default_engine.name].UCI_LimitStrength():
						self.engine_options[chess.WHITE][default_engine.name].UCI_LimitStrength = True
				white_cpu_level_slider_text = sg.Text(Text.ELO_LEVEL, key = Key.WHITE_CPU_LEVEL_SLIDER_TEXT, visible = True)
				white_cpu_level_slider = sg.Slider(range = (self.engine_options[chess.WHITE][default_engine.name].UCI_LimitStrength.min,self.engine_options[chess.WHITE][default_engine.name].UCI_LimitStrength.max), default_value = self.engine_options[chess.WHITE][default_engine.name].UCI_LimitStrength.default, orientation = 'horizontal', enable_events = True, key = Key.WHITE_CPU_LEVEL_SLIDER, disabled = False, visible = True)
			else:
				white_slider_is_invisible = True
				white_cpu_level_slider_text = sg.Text("NOTHING", key = Key.WHITE_CPU_LEVEL_SLIDER_TEXT, visible = True)
				white_cpu_level_slider = sg.Slider(range = (0,2), default_value = 1, orientation = 'horizontal', enable_events = True, key = Key.WHITE_CPU_LEVEL_SLIDER, disabled = False, visible = True)
			white_computer_tab = sg.Tab(Text.CPU, [[white_cpu_combo, sg.Push(), sg.Text(Text.ENGINE_TURN_DURATION, key = Key.WHITE_CPU_DURATION_TEXT), white_cpu_duration],
												[sg.pin(white_cpu_level_slider_text, shrink = False), sg.Push()],
												[sg.pin(white_cpu_level_slider, shrink = False)]],
										key = Key.WHITE_CPU)
		else:
			white_computer_tab = sg.Tab(Text.CPU, [[sg.Text(Text.NO_ENGINE_AVAILABLE)]], key = Key.WHITE_CPU)
		# Network tab definition
		white_network_tab = sg.Tab(Text.NETWORK, [[sg.Text(Text.NOT_YET_IMPLEMENTED)]],
								key = Key.WHITE_NET)
		# Tabgroup definition
		white_tab = sg.TabGroup([[white_human_tab, white_computer_tab, white_network_tab]], key = Key.WHITE_TYPE, enable_events = True)
		
		# Black player tab definition
		# ---------------------------
		# Human tab definition
		black_human_tab = sg.Tab(Text.HUMAN, [[sg.Text(Text.BLACK_PLAYER_NAME)],
		...
		if engine_caption_list:
			...
			for engine in self.engine_list:
				...
				self.engine_options[chess.BLACK][engine.name] = engine.option.copy_without_engine(memo)
				self.log.debug(f"{engine.name} copied.")
			default_engine = self.engines[chess.BLACK][engine_caption_list[0]]
			black_cpu_combo = sg.Combo(engine_caption_list, default_value = default_engine.name, enable_events = True, key = Key.BLACK_ENGINE_COMBO, readonly = True)
			black_cpu_duration = sg.Input(default_text = DEFAULT_SEARCH_TIME, size = 5, key = Key.BLACK_CPU_DURATION)
			...
			if 'UCI_Elo' in default_engine.engine_options:
				if 'UCI_LimitStrength' in default_engine.engine_options:
					if not self.engine_options[chess.BLACK][default_engine.name].UCI_LimitStrength():
						self.engine_options[chess.BLACK][default_engine.name].UCI_LimitStrength = True
				black_cpu_level_slider_text = sg.Text(Text.ELO_LEVEL, key = Key.BLACK_CPU_LEVEL_SLIDER_TEXT, visible = True)
				black_cpu_level_slider = sg.Slider(range = (self.engine_options[chess.BLACK][default_engine.name].UCI_LimitStrength.min,self.engine_options[chess.BLACK][default_engine.name].UCI_LimitStrength.max), default_value = self.engine_options[chess.BLACK][default_engine.name].UCI_LimitStrength.default, orientation = 'horizontal', enable_events = True, key = Key.BLACK_CPU_LEVEL_SLIDER, disabled = False, visible = True)
			else:
				black_slider_is_invisible = True
				black_cpu_level_slider_text = sg.Text("NOTHING", key = Key.BLACK_CPU_LEVEL_SLIDER_TEXT, visible = True)
				black_cpu_level_slider = sg.Slider(range = (0,2), default_value = 1, orientation = 'horizontal', enable_events = True, key = Key.BLACK_CPU_LEVEL_SLIDER, disabled = False, visible = True)
			black_computer_tab = sg.Tab(Text.CPU, [[black_cpu_combo, sg.Push(), sg.Text(Text.ENGINE_TURN_DURATION, key = Key.BLACK_CPU_DURATION_TEXT), black_cpu_duration],
												[sg.pin(black_cpu_level_slider_text, shrink = False), sg.Push()],
												[sg.pin(black_cpu_level_slider, shrink = False)]],
										key = Key.BLACK_CPU)
		else:
			black_computer_tab = sg.Tab(Text.CPU, [[sg.Text(Text.NO_ENGINE_AVAILABLE)]], key = Key.BLACK_CPU)
		# Network tab definition
		black_network_tab = sg.Tab(Text.NETWORK, [[sg.Text(Text.NOT_YET_IMPLEMENTED)]],
								key = Key.BLACK_NET)
		# Tabgroup definition
		black_tab = sg.TabGroup([[black_human_tab, black_computer_tab, black_network_tab]], key = Key.BLACK_TYPE, enable_events = True)
		
		# Frames definitions
		white_frame = sg.Frame(Text.WHITE_PLAYER, [[sg.vtop(sg.Column([[white_logo_image]])), sg.VSep(), white_tab]])
		black_frame = sg.Frame(Text.BLACK_PLAYER, [[sg.vtop(sg.Column([[black_logo_image]])), sg.VSep(), black_tab]])
		
		# Output panel definition
		output_panel = sg.Multiline(autoscroll = True, size = (60, 13), key = Key.OUTPUT, reroute_cprint = True, disabled = True)
		
		# board image definition
		self._board_color = {'square_light':BoardColor.LIGHT_SQUARE, 'square dark':BoardColor.DARK_SQUARE, 'margin':BoardColor.MARGIN, 'coord': BoardColor.COORD}
		board_svg_str = chess.svg.board(self._board, size = self._board_size, colors = self._board_color, flipped = self._board_flipped)
		board_image = self.svg_to_img(board_svg_str, key = Key.BOARD, enable_events = True)
		
		# General buttons definition
		start_button = sg.Button(Text.START, button_color = ("white", "green"), key = Key.START)
		abord_button = sg.Button(Text.ABORD, button_color = ("white", "red"), key = Key.ABORD, visible = False)
		help_button = sg.Button(Text.HELP, button_color = ("white", "green"), key = Key.HELP, visible = False)
		claim_button = sg.Button(Text.CLAIM_FOR_THREEFOLD, button_color = ("white", "orange"), key = Key.CLAIM, visible = False)
		
		# layout definition
		layout = [[sg.vtop(sg.Column([[game_text, game_name, sg.Push(), sg.Column([[player_text, player_image],[sg.Push(), player_name, sg.Push()]])],[white_frame],[black_frame], [start_button, abord_button, help_button, claim_button], [output_panel]])),sg.VSep(),board_image]]
		
		# icon definition
		icon_svg_str = chess.svg.piece(chess.Piece.from_symbol(self.icon_piece), size = self._icon_size)
		icon = self.svg_to_png_bytes(icon_svg_str)
		
		# Window definition
		self.log.debug(f"Attempt to create the main window: DomiChess {self.version if self.version else ''} {Text.POWERED_BY if self._helper_engine else ''} {self._help_engine_caption}")
		self._window = sg.Window(f"DomiChess {self.version if self.version else ''} {Text.POWERED_BY if self._helper_engine else ''} {self._help_engine_caption}", layout, icon = icon, finalize = True, enable_close_attempted_event = True)
		if engine_caption_list:
			if white_slider_is_invisible:
				self._window[Key.WHITE_CPU_LEVEL_SLIDER_TEXT].update(visible = False)
				self._window[Key.WHITE_CPU_LEVEL_SLIDER].update(disabled = True, visible = False)
			if black_slider_is_invisible:
				self._window[Key.BLACK_CPU_LEVEL_SLIDER_TEXT].update(visible = False)
				self._window[Key.BLACK_CPU_LEVEL_SLIDER].update(disabled = True, visible = False)
		...
		if len(self.engine_list) != 0:
			sg.cprint(f"Chess engine{'s' if len(self.engine_list) > 0 else ''} found:", end = " ", t = "green")
			...
			sg.cprint("No chess engine found. Helper and CPU opponents are unavailable.", t = "orange")
	...
	def _init_popups(self, key = Key.ALL_POPUPS):
		...
		if key == Key.ALL_POPUPS or key == sg.WINDOW_CLOSE_ATTEMPTED_EVENT:
			self._confirm_exit_popup = sg.Window(Text.EXIT, [[sg.Text(Text.CONFIRM_EXIT)],
		...
		if key == Key.ALL_POPUPS or key == Key.ABORD:
			self._confirm_abord_popup = sg.Window(Text.ABORD, [[sg.Text(Text.CONFIRM_ABORD)],
															[sg.Yes(Text.YES,key=Key.YES), sg.No(Text.NO, key=Key.NO)]],
												disable_minimize = True, modal = True)
	...
	def _init_engine_list(self, engine_path):
		...
		"""
		engine_list = []
		engine_path_list = [item for item in engine_path.iterdir() if (item.is_file() and (item.suffix.lower() == '.exe'))]
	...
	def board_flipped(self, value):
		if not isinstance(value, bool):
			...
			board_svg_str = chess.svg.board(self.board, size = self._board_size, flipped = self._board_flipped)
	...
	def board(self, value):
		if not isinstance(value, chess.BaseBoard):
			...
			self._board = value
			board_svg_str = chess.svg.board(self._board, size = self._board_size, flipped = self.board_flipped)
			png_bytes = self.svg_to_png_bytes(board_svg_str)
			self._window[Key.BOARD].update(source = png_bytes)
	...
	def _dyn_popup(self, key_popup, params):
		...
		if key_popup == KeyPopup.CHESSMATE:
			...
			if params['winner'] == chess.WHITE:
				logo_svg_str = chess.svg.piece(chess.Piece.from_symbol(self._white_logo_piece), size = self._logo_size)
				logo_image = self.svg_to_img(logo_svg_str, key = Key.WHITE_LOGO)
			else:
				logo_svg_str = chess.svg.piece(chess.Piece.from_symbol(self._black_logo_piece), size = self._logo_size)
				logo_image = self.svg_to_img(logo_svg_str, key = Key.BLACK_LOGO)
			result = sg.Window(Text.GAME_ENDED, [[logo_image, sg.Column([[sg.Text(Text.CHESSMATE)],[sg.Text(Text.SOMEONE_WINS.format(params['name_winner']))]])],
												[sg.Push(),sg.Ok(Text.OK,key=Key.OK),sg.Push()]],
								disable_minimize = True, modal = True).read(close=True)
			...
			if params['draw'] == Draw.STALEMATE:
				...
				text_to_display = ""
			result = sg.Window(Text.GAME_ENDED, [[white_logo_image, black_logo_image, sg.Column([[sg.Text(Text.DRAW)],[sg.Text(text_to_display)]])],
												[sg.Push(),sg.Ok(Text.OK,key=Key.OK),sg.Push()]],
								disable_minimize = True, modal = True).read(close=True)
		elif key_popup == KeyPopup.PROMOTION:
			self.log.debug("displaying promotion popup.")
			if params['player'] == chess.WHITE:
				rook_svg_str = chess.svg.piece(chess.Piece.from_symbol("R"), size = self._logo_size)
				knight_svg_str = chess.svg.piece(chess.Piece.from_symbol("N"), size = self._logo_size)
				bishop_svg_str = chess.svg.piece(chess.Piece.from_symbol("B"), size = self._logo_size)
				queen_svg_str = chess.svg.piece(chess.Piece.from_symbol("Q"), size = self._logo_size)
			else:
				rook_svg_str = chess.svg.piece(chess.Piece.from_symbol("r"), size = self._logo_size)
				knight_svg_str = chess.svg.piece(chess.Piece.from_symbol("n"), size = self._logo_size)
				bishop_svg_str = chess.svg.piece(chess.Piece.from_symbol("b"), size = self._logo_size)
			...
			knight_image = self.svg_to_img(knight_svg_str, key = Key.KNIGHT, enable_events = True)
			bishop_image = self.svg_to_img(bishop_svg_str, key = Key.BISHOP, enable_events = True)
			queen_image = self.svg_to_img(queen_svg_str, key = Key.QUEEN, enable_events = True)
			result = sg.Window(Text.PROMOTION, [[sg.Push(), sg.Text(Text.WHAT_PROMOTION), sg.Push()],
												[sg.Push(),rook_image,knight_image,bishop_image,queen_image,sg.Push()]],
								disable_minimize = True, modal = True).read(close=True)
	...
	def _coord_to_square(self, coord):
		...
		current_margin_size = int(CHESS_DEFAULT_MARGIN_SIZE * self._board_size / CHESS_DEFAULT_BOARD_SIZE)
		current_square_size = int(CHESS_DEFAULT_SQUARE_SIZE * self._board_size / CHESS_DEFAULT_BOARD_SIZE)
		
		# File (column) calculation
		if (x < current_margin_size) or (x > (current_margin_size + 8*current_square_size)):
			return None
		else:
		...
		if (y < current_margin_size) or (y > (current_margin_size + 8*current_square_size)):
			return None
		else:
			if self._board_flipped:
	...
	def _prepare_game(self, white_name, white_type, black_name, black_type, values):
		...
		if white_type == Key.WHITE_HUMAN:
			...
			type = Type.CPU
			original_engine = [engine for engine in self.engine_list if engine.name == values[Key.WHITE_ENGINE_COMBO]][0]
			memo = dict()
			self.engines[chess.WHITE][values[Key.WHITE_ENGINE_COMBO]] = deepcopy(original_engine, memo)
			self._apply_engine_options(self.engine_options[chess.WHITE][values[Key.WHITE_ENGINE_COMBO]], self.engines[chess.WHITE][values[Key.WHITE_ENGINE_COMBO]])
			
		elif white_type == Key.WHITE_NET:
			type = Type.NETWORK
		else:
			type = Type.HUMAN
		white_player = Player(name = white_name, color = chess.WHITE, type = type, logger = self.log)
		...
		if black_type == Key.BLACK_HUMAN:
			...
			type = Type.CPU
			original_engine = [engine for engine in self.engine_list if engine.name == values[Key.BLACK_ENGINE_COMBO]][0]
			memo = dict()
			self.engines[chess.BLACK][values[Key.BLACK_ENGINE_COMBO]] = deepcopy(original_engine, memo)
			self._apply_engine_options(self.engine_options[chess.BLACK][values[Key.BLACK_ENGINE_COMBO]], self.engines[chess.BLACK][values[Key.BLACK_ENGINE_COMBO]])
		elif black_type == Key.BLACK_NET:
			type = Type.NETWORK
		else:
			type = Type.HUMAN
		black_player = Player(name = black_name, color = chess.BLACK, type = type, logger = self.log)
		...
		# Game
		return Game(name = values[Key.CURRENT_GAME_NAME], white_player = white_player, black_player = black_player, helper_engine = self._helper_engine, logger = self.log)
	...
	def _update_cpu_tab(self, player, engine_name):
		...
		if 'UCI_Elo' in self.engines[player][engine_name].engine_options:
			...
			self._window[KEY_CPU_LEVEL_SLIDER_TEXT].update(value = Text.ELO_LEVEL, visible = True)
			self._window[KEY_CPU_LEVEL_SLIDER].update(value = self.engine_options[player][engine_name].UCI_Elo(), range = (self.engine_options[player][engine_name].UCI_Elo.min,self.engine_options[player][engine_name].UCI_Elo.max) , disabled = False, visible = True)
		else:
		...
		while True:
			...
			if event == sg.WINDOW_CLOSE_ATTEMPTED_EVENT: # Close the application
				if self._confirm_exit_popup.read(close=True)[0] == Key.YES:
					if hasattr(self, "_game"): self._game.ongoing = False
					...
					self._init_popups(event) # popup window re-initialized (to forget the result in a further call)
				...
				if values[Key.WHITE_TYPE] == Key.WHITE_HUMAN:
					...
					if self._helper_engine is None: # If no helper engine, then no engine at all
						self._window[Key.START].update(disabled=True)
					else:
						self._window[Key.START].update(disabled=False)
					if values[Key.BLACK_TYPE] == Key.BLACK_HUMAN:	# Flipped or not flipped ?
						self.board_flipped = True
					else:
						self.board_flipped = False
				elif values[Key.WHITE_TYPE] == Key.WHITE_NET:
					# self.log.debug("White player is a remote opponent.")
					self._window[Key.START].update(disabled=True) # Network game not yet implemented
					if values[Key.BLACK_TYPE] == Key.BLACK_HUMAN:	# Flipped or not flipped ?
						self.board_flipped = True
					else:
						self.board_flipped = False
				...
				if values[Key.BLACK_TYPE] == Key.BLACK_HUMAN:
					...
					self._window[Key.START].update(disabled=False)
					if values[Key.WHITE_TYPE] == Key.WHITE_HUMAN:	# Flipped or not flipped ?
						self.board_flipped = False
					else:
						self.board_flipped = True
			...
			elif event == Key.WHITE_ENGINE_COMBO: # Change the engine for the CPU white player
				player = chess.WHITE
				engine_name = values[Key.WHITE_ENGINE_COMBO]
				self._update_cpu_tab(player, engine_name)
			
			elif event == Key.BLACK_ENGINE_COMBO: # Change the engine for the CPU black player
				player = chess.BLACK
				engine_name = values[Key.BLACK_ENGINE_COMBO]
				self._update_cpu_tab(player, engine_name)
			
			elif event == Key.WHITE_CPU_LEVEL_SLIDER: # Change the level for the CPU white player
				engine_name = values[Key.WHITE_ENGINE_COMBO]
				...
				if 'UCI_Elo' in self.engines[chess.WHITE][engine_name].engine_options:
					if 'UCI_LimitStrength' in self.engines[chess.WHITE][engine_name].engine_options:
						self.engine_options[chess.WHITE][engine_name].UCI_LimitStrength = True
					# self.log.debug(f"{engine_name}: set UCI_Elo to {values[Key.WHITE_CPU_LEVEL_SLIDER]} for the white player")
					self.engine_options[chess.WHITE][engine_name].UCI_Elo = values[Key.WHITE_CPU_LEVEL_SLIDER]
					
			
			elif event == Key.BLACK_CPU_LEVEL_SLIDER: # Change the level for the CPU black player
				engine_name = values[Key.BLACK_ENGINE_COMBO]
				...
				if 'UCI_Elo' in self.engines[chess.BLACK][engine_name].engine_options:
					if 'UCI_LimitStrength' in self.engines[chess.BLACK][engine_name].engine_options:
						self.engine_options[chess.BLACK][engine_name].UCI_LimitStrength = True
					# self.log.debug(f"{engine_name}: set UCI_Elo to {values[Key.BLACK_CPU_LEVEL_SLIDER]} for the black player")
					self.engine_options[chess.BLACK][engine_name].UCI_Elo = values[Key.BLACK_CPU_LEVEL_SLIDER]
				...
				if values[Key.WHITE_TYPE] == Key.WHITE_HUMAN:
					...
					if 'UCI_Elo' in self.engines[chess.WHITE][white_name].engine_options:
						sg.cprint(f"ELO = {self.engines[chess.WHITE][white_name].option.UCI_Elo()},", end = " ", t = "black")
						self.log.debug(f"ELO = {self.engines[chess.WHITE][white_name].option.UCI_Elo()}")
					self.engines[chess.WHITE][white_name].search_time = float(values[Key.WHITE_CPU_DURATION])
					sg.cprint(f"turn duration = {self.engines[chess.WHITE][white_name].search_time} s.", t = "black")
					self.log.debug(f"Turn duration = {self.engines[chess.WHITE][white_name].search_time} s.")
				...
				if values[Key.BLACK_TYPE] == Key.BLACK_HUMAN:
					...
					if 'UCI_Elo' in self.engines[chess.BLACK][black_name].engine_options:
						sg.cprint(f"ELO = {self.engines[chess.BLACK][black_name].option.UCI_Elo()},", end = " ", c = "white on black")
						self.log.debug(f"ELO = {self.engines[chess.BLACK][black_name].option.UCI_Elo()}")
					self.engines[chess.BLACK][black_name].search_time = float(values[Key.BLACK_CPU_DURATION])
					sg.cprint(f"turn duration = {self.engines[chess.BLACK][black_name].search_time} s.", c = "white on black")
					self.log.debug(f"Turn duration = {self.engines[chess.BLACK][black_name].search_time} s.")
				...
				self._game = self._prepare_game(white_name = white_name, white_type = values[Key.WHITE_TYPE], black_name = black_name, black_type = values[Key.BLACK_TYPE], values = values)
				self._game.ongoing = True
				
				self._window[Key.CURRENT_PLAYER_NAME].update(self._game.players[chess.WHITE].name)
				self._window[Key.CURRENT_PLAYER_IMAGE].update(self._current_white_png_bytes)
				...
				if values[Key.WHITE_TYPE] == Key.WHITE_CPU: # If the white player is an engine, start it
					self._white_engine = self.engines[chess.WHITE][white_name]
					self._white_engine.start_new_game(values[Key.CURRENT_GAME_NAME])
					self._window.perform_long_operation(lambda:self._white_engine.next_move(self._game.board), Key.CPU_OR_REMOTE_MOVE)
				...
				self.log.debug(f"{self._game.players[self._game.current_color].name} asks for help.")
				self._window.perform_long_operation(self._game.ask_for_help, Key.REPLY_TO_HELP)
				self._window[Key.HELP].update(disabled=True)
			
			elif event == Key.CLAIM: # Claim for a draw
				if self._draw_claimed == Draw.THREEFOLD:
					key_popup = KeyPopup.DRAW
					params = {'draw':Draw.THREEFOLD, 'player':self._game.players[self._game.current_color].name}
				elif self._draw_claimed == Draw.FIFTY:
					key_popup = KeyPopup.DRAW
					params = {'draw':Draw.FIFTY, 'player':self._game.players[self._game.current_color].name}
				...
				if hasattr(self, "_game") and self._game.ongoing: # Only if a game is ongoing
					user_event = self._window[Key.BOARD].user_bind_event
					square = self._coord_to_square((user_event.x,user_event.y))
					self.log.debug(f"Left click at ({user_event.x},{user_event.y}): square {chess.square_name(square) if square is not None else None}")
					
					if self.board.color_at(square) is not None and (self.board.color_at(square) == self._game.current_color) and (self._game.players[self._game.current_color].type == Type.HUMAN):
						if self._start_pos is None:
							self._reachable_squares = self._game.reaching_squares_from_pos(square)
							self.log.debug(f"Reachable squares: {tuple(chess.square_name(reachable) for reachable in self._reachable_squares)}")
							if len(self._reachable_squares) > 0:
								fill = {square:BoardColor.START_SQUARE} | dict.fromkeys(self._reachable_squares, BoardColor.REACHABLE_SQUARE)
								board_svg_str = chess.svg.board(self.board,
																fill = fill,
						...
						self.log.debug(f"Destination chosen: {chess.square_name(square)}")
						...
						if self.board.piece_type_at(self._start_pos) == chess.PAWN:
							if (self._game.current_color==chess.WHITE and chess.square_rank(square)==7) or (self._game.current_color==chess.BLACK and chess.square_rank(square)==0):
								promotion_key, _ = self._dyn_popup(key_popup = KeyPopup.PROMOTION, params = {'player':self._game.current_color})
						...
						result = self._game.add_move(chess.Move(from_square = self._start_pos, to_square = self._dest_pos, promotion = promotion))
						sg.cprint(f"{result.san}", end=" ")
						self.board = self._game.current_board
						
						if game_result := self._game.result_with_claim: # the game has ended with a chessmate or a draw without a claim, or a draw can be claimed
							self.log.debug(f"The game terminates (or a draw is claimable) on the following result code: {game_result.termination}")
							if game_result.termination == chess.Termination.CHECKMATE:
								self.log.info(f"Chessmate ! The {'white' if game_result.winner else 'black'} player {self._game.players[game_result.winner].name} wins the game.")
								key_popup = KeyPopup.CHESSMATE
								params = {'winner':game_result.winner, 'name_winner':self._game.players[game_result.winner].name}
								game_is_finished = True
							elif game_result.termination == chess.Termination.STALEMATE:
								self.log.info(f"Stalemate: {'white' if self._game.current_color else 'black'} player has no possible legal move ! No winner.")
								key_popup = KeyPopup.DRAW
								params = {'draw':Draw.STALEMATE, 'player':self._game.players[self._game.current_color].name}
								game_is_finished = True
							elif game_result.termination == chess.Termination.INSUFFICIENT_MATERIAL:
								self.log.info(f"{'White' if self._game.current_color else 'Black'} player has no sufficient material to win ! No winner.")
								key_popup = KeyPopup.DRAW
								params = {'draw':Draw.INSUFFICIENT_MATERIAL, 'player':self._game.players[self._game.current_color].name}
								game_is_finished = True
							elif game_result.termination == chess.Termination.SEVENTYFIVE_MOVES:
								self.log.info("No capture or no pawn move has occured in the last 75 moves ! No winner.")
								key_popup = KeyPopup.DRAW
								params = {'draw':Draw.SEVENTY_FIVE}
								game_is_finished = True
							elif game_result.termination == chess.Termination.FIVEFOLD_REPETITION:
								self.log.info("An identical position occurs five times ! No winner.")
								key_popup = KeyPopup.DRAW
								...
								self._window[Key.CLAIM].update(text = Text.CLAIM_FOR_FIFTY, visible=True, disabled=False)
								game_is_finished = False
							elif game_result.termination == chess.Termination.THREEFOLD_REPETITION:
								key_popup = None # The popup window will be called later, if the draw is claimed
								self._draw_claimed = Draw.THREEFOLD
								self._window[Key.CLAIM].update(text = Text.CLAIM_FOR_THREEFOLD, visible=True, disabled=False)
								game_is_finished = False
							else:
								key_popup = None
								game_is_finished = False
							...
							if game_is_finished: # Game is finished: re-init the board
								...
								self._window[Key.CURRENT_PLAYER_NAME].update(self._game.players[self._game.current_color].name)
								self._window[Key.CURRENT_PLAYER_IMAGE].update(self._current_white_png_bytes if self._game.current_color else self._current_black_png_bytes)
						else:
							self._window[Key.CURRENT_PLAYER_NAME].update(self._game.players[self._game.current_color].name)
							self._window[Key.CURRENT_PLAYER_IMAGE].update(self._current_white_png_bytes if self._game.current_color else self._current_black_png_bytes)
							if self._helper_engine is not None:
								if self._game.players[self._game.current_color].type == Type.HUMAN:
									self._window[Key.HELP].update(visible=True, disabled=False)
								else:
									self._window[Key.HELP].update(visible=False, disabled=True)
							else:
						...
						if self._game.players[self._game.current_color].type == Type.CPU and self._game.ongoing:
							if self._game.current_color == chess.WHITE:
								try:
									self._window.perform_long_operation(lambda:self._white_engine.next_move(self.board), Key.CPU_OR_REMOTE_MOVE)
								except chess.engine.EngineTerminatedError: # The engine is already closed (if the game is aobrded for instance)
									self.log.debug("White engine attempts to make a move but the game is already finished.")
							else:
								try:
									self._window.perform_long_operation(lambda:self._black_engine.next_move(self.board), Key.CPU_OR_REMOTE_MOVE)
								except chess.engine.EngineTerminatedError: # The engine is already closed (if the game is aobrded for instance)
									self.log.debug("Black engine attempts to make a move but the game is already finished.")
							
			
			elif event == Key.REPLY_TO_HELP: # Reception of the reply for asking to help
				if values[event]:
					self.log.debug(f"The best move according to the helper engine is: from {chess.square_name(values[event].from_square)} to {chess.square_name(values[event].to_square)}")
					arrows = [chess.svg.Arrow(values[event].from_square, values[event].to_square, color = BoardColor.ARROW)]
					board_svg_str = chess.svg.board(self.board,
													arrows = arrows,
													size = self._board_size,
													flipped = self.board_flipped)
			...
			elif event == Key.CPU_OR_REMOTE_MOVE: # Reception of the move of the local engine or a remote opponent
				if not self._game.ongoing: # If the has been aborded during the previous analysis
					...
					self.log.debug(f"Engine or remote opponent move: from {chess.square_name(values[event].from_square)} to {chess.square_name(values[event].to_square)}")
