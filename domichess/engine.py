...
# Global constants
#-----------------
UCI_TIMEOUT = 2.0 # Time out (in seconds) when asking either the engine is uci-compatible or not
...
class OptionItem:
	...
	def __deepcopy__(self, memo):
		return OptionItem(name = deepcopy(self.name, memo), type = deepcopy(self.type, memo), default = deepcopy(self.default, memo), min = deepcopy(self.min, memo), max = deepcopy(self.max, memo), var = deepcopy(self.var, memo), value = deepcopy(self.value, memo))
	...
	def copy(self):
		...
		"""
		return OptionItem(name = self.name, type = self.type, default = self.default, min = self.min, max = self.max, var = deepcopy(self.var), value = self.value)
...
class Options():
	...
	def __dir__(self):
		...
		"""
		return [attr for attr in self.__dict__.keys() if not (attr.startswith('__') | (attr == 'log') | (attr == 'chess_simple_engine') | (attr == 'copy') | (attr == 'copy_without_engine'))]
				...
				if option.type == UCIOptionType.CHECK:
					...
					if isinstance(value, int) | isinstance(value, float):
						if (value >= option.min) and (value <= option.max):
							...
							raise ValueError(f"Expected {value} to be between {option.min} and {option.max}")
					else:
						raise ValueError(f"Expected {value} to be an integer or a float")
					...
					if self.chess_simple_engine:
						...
						self.log.debug(f"Engine {self.chess_simple_engine.id['name']} updated: Button type option {option.name} switched")
				else:
					ok_for_update = True
					formatted_value = value
				if ok_for_update:
					option.value = value
					if self.chess_simple_engine:
						if not self.chess_simple_engine.options[option.name].is_managed():
							self.chess_simple_engine.configure({option.name:formatted_value})
							self.log.debug(f"Engine {self.chess_simple_engine.id['name']} updated: Option {option.name} set to {formatted_value}")
			else:
				raise ValueError(f"{name} is not an option for the engine {self.chess_simple_engine.id['name']}")
...
class Engine():
	...
	def __init__(self, engine_path, logger = None, no_options = False):
		...
		self.protocol = EngineProtocol.UNKNOWN # Default value of self.protocol if an exception raises
	...
	def __del__(self):
		...
		"""
		self.log.debug(f"Attempt to unregister {self.name if self.protocol != EngineProtocol.UNKNOWN else self._engine_path.name} engine")
		# del self.option
		self.quit()
		self.log.debug(f"{self.name if self.protocol != EngineProtocol.UNKNOWN else self._engine_path.name} engine successfully unregistred")
	...
	def _is_uci_compatible(self):
		...
		"""
		try:
			self.chess_simple_engine = chess.engine.SimpleEngine.popen_uci(self._engine_path, timeout = COMMAND_TIMEOUT)
		except chess.engine.EngineError:
			self.log.warning(f"UCI Protocol error: {self._engine_path.name} seems not to be a compatible chess engine. Please consider deleting it from the engines directory.")
			return False
		else:
			...
			if not self._no_options:
				...
				for option in self.chess_simple_engine.options.values():
					if not option.is_managed():
						...
						setattr(self.option, formatted_name, OptionItem(name = option.name, type = option.type, default = option.default, min = option.min, max = option.max, var = option.var if option.var else [], value = option.default))
						self.log.debug(f"{self.name} engine: option {option.name} added, with default value {option.default}")
			return True					
	...
	def next_move(self, board):
		...
		"""
		return self.chess_simple_engine.play(board, chess.engine.Limit(time=self.search_time), game = self.game_name).move
	...
	def reopen(self):
		...
		if self.engine_is_closed:
			try:
				...
				self.log.debug(f"{self.name} engine is re-opened: the options are all set to their previously modified values.")
				memo = dict()
				recorded_options = deepcopy(self.option, memo)
				for option in self.engine_options: # Apply the recorded option values
					setattr(self.option, option, getattr(recorded_options, option).value)
		else:
			self.log.debug(f"{self._engine_path.name} seems to be already open. It is not necessary to re-open it.")
			return False
