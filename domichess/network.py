	...
	def handle(self):
		...
		if self.client_address[0] != self.server.local_address:
			...
			if recv_opcode == opcode.IAMHERE:
				self.server.log.debug(f"Receipt of a declaration of a new player from {self.client_address[0]}")
				if self.client_address[0] not in self.server.play_addr:
					self.server.log.info(f"{self.client_address[0]} added to the list of the remote players")
					with self.server.addr_lock:
						self.server.play_addr.append(self.client_address[0])
					self.server.log.debug(f"Reply to {self.client_address[0]}: declared as a new player")
					# Reply to the remote server and not the client since the latter has not been listening to anymore
					sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
					sock.sendto(bytes([opcode.IAMHERE]), (self.client_address[0], self.server.port))
				else:
					self.server.log.debug(f"{self.client_address[0]} is already known as an available player")
			elif recv_opcode == opcode.KEEPALIVE:
				self.server.log.debug(f"Receipt of a keep alive from {self.client_address[0]}")
				...
				self.server.log.info(f"Name of the player {self.client_address[0]} is: {name}")
				if self.client_address[0] in self.server.remote_players:
					self.server.log.debug(f"the name of the player {self.client_address[0]} is updated")
					...
					self.server.log.debug(f"The player {self.client_address[0]} is created")
					with self.server.players_lock:
				...
				self.server.log.info(f"UUID of the player {self.client_address[0]} is: {uuid}")
				if self.client_address[0] in self.server.remote_players:
					player = self.server.remote_players[self.client_address[0]]
					if player.uuid == uuid:
						self.server.log.debug(f"The player {self.client_address[0]} is already registred with this UUID")
					else:
						player = Player(type = Type.NETWORK, uuid = uuid)
						self.server.log.debug(f"The player {self.client_address[0]} is re-created with a new UUID")
						with self.server.players_lock:
							self.server.remote_players[self.client_address[0]] = player
				else:
					player = Player(type = Type.NETWORK, uuid = uuid)
				...
				self.server.log.info(f"Color of the player {self.client_address[0]} is: {'WHITE' if color else 'BLACK'}")
				if self.client_address[0] in self.server.remote_players:
					self.server.log.debug(f"the color of the player {self.client_address[0]} is updated")
					with self.server.players_lock:
				...
				self.server.log.info(f"The characteristics of the player {self.client_address[0]} are:\n    UUID: {player_full['uuid']}\n    Name: {player_full['name']}\n    Color: {'WHITE' if player_full['color'] else 'BLACK'}")
				player = Player(name=player_full['name'], color=player_full['color'], type=Type.NETWORK, uuid=player_full['uuid'])
				with self.server.players_lock:
					self.server.remote_players[self.client_address[0]] = player
			elif recv_opcode == opcode.GAMENAME:
				game_name = json.loads(str(data[1:], "utf-8"))
				self.server.log.info(f"Name of the game hosted by {self.client_address[0]} is: {game_name}")
				if self.client_address[0] in self.server.remote_games:
					self.server.log.debug(f"The name of the game hosted by {self.client_address[0]} is updated")
					with self.server.games_lock:
						self.server.remote_games[self.client_address[0]]["name"] = game_name
				else:
					game = {"name":game_name, "uuid":str(uuid4())}
					self.server.log.debug(f"The game hosted by {self.client_address[0]} is created")
					with self.server.players_lock:
						self.server.remote_games[self.client_address[0]] = game
			elif recv_opcode == opcode.GAMEUUID:
				game_uuid = json.loads(str(data[1:], "utf-8"))
				self.server.log.info(f"UUID of the game hosted by {self.client_address[0]} is: {game_uuid}")
				if self.client_address[0] in self.server.remote_games:
					self.server.log.debug(f"The UUID of the game hosted by {self.client_address[0]} is updated")
					with self.server.games_lock:
						self.server.remote_games[self.client_address[0]]["uuid"] = game_uuid
				else:
				...
				self.server.log.info(f"Player {self.client_address[0]} wants to join the local game")
				with self.server.joined_lock:
					self.server.joined_players[self.client_address[0]] = True
			elif recv_opcode == opcode.GAMELEAVE:
				self.server.log.info(f"Player {self.client_address[0]} wants to leave the local game")
				...
				self.server.log.info(f"The player {self.client_address[0]} is leaving the network")
				if self.client_address[0] in self.server.play_addr:
					with self.server.addr_lock:
						del self.server.play_addr[self.client_address[0]]
				if self.client_address[0] in self.server.remote_players:
					with self.server.players_lock:
...
class LANFinderServer(ThreadingUDPServer):
	...
	def __init__(self, port:int, server_port:int|None = None, logger:logging.Logger|None = None):
		...
		super().__init__(('', ser_port), RequestHandlerClass = FinderHandler, bind_and_activate = True)
	...
	def find_remote_players(self):
		...
		"""
		sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		self._log.debug("Send an IAMHERE message over the LAN through the broadcast address")
		try:
			for broad in self.broad_addr:
				sock.sendto(bytes([opcode.IAMHERE]), (broad, self.port))
		except PermissionError:
			self._log.exception(f"{broad} broadcast address is not permitted on this interface by Windows. Please grant access and try again.")
			sys.exit(0)
	...
	def check_one_remote(self, addr:str, addr_to_remove:list, lock:threading.Lock):
		...
		try:
			...
			if reply[0][0] != opcode.KEEPALIVE:
				with lock:
					self._log.debug(f"Bad answer to keep alive: {addr} is not yet available")
	...
	def check_remote_players(self):
		...
		for addr in self.play_addr:
			threads.append(threading.Thread(target=self.check_one_remote, kwargs={"addr":addr,"addr_to_remove":addr_to_remove,"lock":lock}))
		...
		if len(addr_to_remove) > 0:
			with self.addr_lock:
				self._log.info(f"The following addresses are removed from the list of the remote players: {addr_to_remove}")
				for addr in addr_to_remove:
					self.play_addr.remove(addr)
		
		self._log.debug(f"The current available players are:\n    {self._local_address} (local)\n    {self._play_addr}")
	...
	def send_once_player_characteristics(self, addr:str, player:Player):
		...
		if addr in self.play_addr:
			...
			sock.sendto(bytes([opcode.PLAYERFULL]) + bytes(serialized_player, "utf-8"), (addr, self.port))
		else:
			raise ValueError(f"{addr} is not a correct player's address")")
	
	def send_player_characteristics(self, player:Player):
		"""
		Sends player characteristics to all known remote players aver the LAN.
	...
	def send_once_player_update(self, addr:str, charact:opcode, value:str|bool):
		...
		"""
		if (addr in self.play_addr) and (charact in [opcode.PLAYERNAME, opcode.PLAYERUUID, opcode.PLAYERCOLOR]):
			if ((charact in [opcode.PLAYERNAME, opcode.PLAYERUUID]) and isinstance(value, str)) or (charact == opcode.PLAYERCOLOR and isinstance(value, bool)):
				serialized_value = json.dumps(value)
				sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
				self._log.debug(f"Send to {addr} the {'color' if charact==opcode.PLAYERCOLOR else 'UUID' if charact==opcode.PLAYERUUID else 'name'}: {'WHITE' if (charact==opcode.PLAYERCOLOR and value) else 'BLACK' if (charact==opcode.PLAYERCOLOR and not value) else value}")
				sock.sendto(bytes([charact]) + bytes(serialized_value, "utf-8"), (addr, self.port))
			else:
	...
	def send_once_game_charact(self, addr:str, charact:opcode, value:str):
		...
		Raises:
			...
			TypeError:
				If `value` does not meet the suitable type
		"""
		if (addr in self.play_addr) and (charact in [opcode.GAMENAME, opcode.GAMEUUID]) and (isinstance(value, str)):
			serialized_value = json.dumps(value)
			sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			self._log.debug(f"Send to {addr} the {'name' if charact==opcode.GAMENAME else 'UUID'} of the game: {value}")
			sock.sendto(bytes([charact]) + bytes(serialized_value, "utf-8"), (addr, self.port))
		else:
			if addr not in self.play_addr:
				raise ValueError(f"{addr} is not a correct player's address")")
	...
	def send_game_charact(self, charact:opcode, value:str):
		...
		if (charact in [opcode.GAMENAME, opcode.GAMEUUID]) and (isinstance(value, str)):
			for addr in self.play_addr:
				self.send_game_charact(addr, charact, value)
		else:
	...
	def join_game(self, addr:str):
		...
		if addr in self.play_addr:
			sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			self._log.debug(f"Inform {addr} that we wish to join his game named {self.remote_games[addr]}")
	...
	def leave_game(self, addr:str):
		...
		if addr in self.play_addr:
			sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			self._log.debug(f"Inform {addr} that we wish to leave his game named {self.remote_games[addr]}")
	...
	def start_game(self, addr:str):
		...
		if (addr in self.play_addr) and (addr in self.joined_players):
			...
			if addr not in self.play_addr:
				...
				raise ValueError(f"{addr} is a known player, but does not wish to play with us")
...
def get_broadcast_address():
    """
	Gets the broadcast addresses of all interfaces supporting the IP protocol.
	"""
	
	
	
	# Obtenez le nom d'hôte de l'ordinateur
    host_name = socket.gethostname()

    # Obtenez l'adresse IP de l'ordinateur
    ip_address = socket.gethostbyname(host_name)

    # Obtenez le masque de sous-réseau de l'interface réseau par défaut
    # Cette ligne de code ne fonctionne que sous Linux et MacOS. Sous Windows, utilisez getsockopt() à la place.
    netmask = socket.inet_ntoa(struct.pack('!I', socket.if_nameindex()[1][3]))
    ...
    # Appliquez le masque de sous-réseau à l'adresse IP pour obtenir l'adresse de réseau
    network_address = struct.pack('!I', struct.unpack('!I', ip_bytes)[0] & struct.unpack('!I', netmask_bytes)[0])

    # Calculez l'adresse de diffusion en appliquant le complément binaire de l'octet de masque de sous-réseau
    # à chaque octet de l'adresse de réseau
    broadcast_address = socket.inet_ntoa(struct.pack('!I', struct.unpack('!I', network_address)[0] | (~struct.unpack('!I', netmask_bytes)[0] & 0xffffffff)))
