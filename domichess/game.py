...
class Player():
	...
	def __init__(self, name = "Player", color = chess.WHITE, type = Type.HUMAN, uuid = None, logger = None):
		"""
		...
		self.log.debug(f"--- {'White' if self._color else 'Black'} player initialization ---")
	...
	def name(self, value):
		if not isinstance(value, str):
			...
			self.log.debug(f"The name of the {'white' if self.color else 'black'} player has been changed into {value}.")
			self._name = value
	...
	def color(self, value):
		if not isinstance(value, bool):
			...
			self.log.debug(f"{self.name} is now the {'white' if value else 'black'} opponent.")
			self._color = value
	...
	def type(self, value):
		if value not in (Type.HUMAN, Type.CPU, Type.NETWORK):
			...
			if value == Type.HUMAN:
				self.log.debug(f"The {'white' if self.color else 'black'} player {self.name} is now a local human.")
			elif value ==Type.CPU:
				self.log.debug(f"The {'white' if self.color else 'black'} player {self.name} is now the local CPU.")
			else:
				self.log.debug(f"The {'white' if self.color else 'black'} player {self.name} is now a remote opponent.")
			self._type = value
...
class Game():
	...
	def __init__(self, name = "game", white_player = None, black_player = None, helper_engine = None, uuid = None, logger = None):
		"""
		...
		self.players = PlayerDict({chess.WHITE: white_player, chess.BLACK: black_player})
	...
	def reaching_squares_from_pos(self, start_pos):
		...
		"""
		reachable_squares = tuple(move.to_square for move in self._board.legal_moves if move.from_square == start_pos)
